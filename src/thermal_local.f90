...
#include "compilation_info.f90"
 ...
 parsingFile: do while (trim(line) /= IO_EOF)                                                       ! read through sections of homogenization part
   ...
   if (IO_getTag(line,'[',']') /= '') then                                                          ! next section
     homog = homog + 1_pInt
     if (thermal_type(homog) == THERMAL_local_ID) then
   ...
   if (homog > 0_pInt ) then                                                                      ! do not short-circuit here (.and. with next if-statement). It's not safe in Fortran
     if (thermal_type(homog) == THERMAL_local_ID) then                         ! one of my sections
...
! allocate state arrays
     ...
     thermalState(homog)%sizePostResults = thermal_local_sizePostResults(instance)
