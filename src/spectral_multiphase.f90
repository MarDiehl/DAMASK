...
! constructing residual
 ...
 call DMDAVecGetArrayF90(da_local,solution_current_local,solution_current_scal,ierr)                ! sol_cur_local(PETSc, chem potential) -> sol_cut_scal(our) ...
 CHKERRQ(ierr)
 call DMDAVecGetArrayF90(da_local,solution_lastInc,      solution_lastInc_scal,ierr)                ! concvec_lastInc(PETSc) -> sol_last_scal(our, but concentration, not chem pot!)
 CHKERRQ(ierr)
 cell = 0
 do k = zstart, zend; do j = ystart, yend; do i = xstart, xend
   cell = cell + 1
   solution_current_elem(1,1:NActivePhases) = solution_current_scal(0:NActivePhases-1,i  ,j  ,k  )   ! chem pots at three neighboring piints for differetiation
   ...
   phi_current     (  1:NActivePhases) = solution_current_scal(0:NActivePhases-1,i  ,j  ,k  )                 ! chem pot -> conc vector
   phi_lastInc     (  1:NActivePhases) = solution_lastInc_scal(0:NActivePhases-1,i  ,j  ,k  )                 ! chem pot -> conc vector

   phi_grad = matmul(BMat,solution_current_elem)
   phi_flux   = homogenization_multiphase_getPhaseFlux  (phi_grad,phi_current,1,cell)
...
subroutine spectral_multiphase_formJacobian(da_local,solution_current_local,Jac_pre,Jac,dummy,ierr)
 ...
 real(pReal)          :: phi_sourceTangent(    NActivePhases,NActivePhases), &
                         phi_fluxTangent  (3,3,NActivePhases,NActivePhases)
...
! assemble matrix
 ...
 do k = zstart, zend; do j = ystart, yend; do i = xstart, xend
   ...
   phi_current(1:NActivePhases) = solution_current_scal(0:NActivePhases-1,i,j,k)                 ! chem pot -> conc vector
   phi_fluxTangent   = homogenization_multiphase_getPhaseFluxTangent  (phi_current,1,cell)
   ...
   do phaseI = 0, NActivePhases-1; do phaseJ = 0, NActivePhases-1
     k_ele(4*phaseI+1:4*(phaseI+1),4*phaseJ+1:4*(phaseJ+1)) = &
       k_ele(4*phaseI+1:4*(phaseI+1),4*phaseJ+1:4*(phaseJ+1)) + &
       params%timeinc* &
       matmul(transpose(BMat),matmul(phi_fluxTangent(1:3,1:3,phaseI,phaseJ),BMat))
   enddo; enddo  
   k_ele = transpose(k_ele)
   call MatSetValuesStencil(Jac,4*NActivePhases+4,col,4*NActivePhases+4,col,k_ele,ADD_VALUES,ierr)
   CHKERRQ(ierr)
 enddo; enddo; enddo
